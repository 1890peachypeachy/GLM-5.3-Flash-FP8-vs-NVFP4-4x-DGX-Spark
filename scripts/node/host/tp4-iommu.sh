...
#   1. `./scripts/tp4ctl down` first — the cluster must be stopped, no rank left running.
...
do_apply() {
  ...
  if [ ! -f "$DROPIN" ]; then
    if [ -e "$SENTINEL" ]; then
      ...
      warn "$DROPIN is not installed: run scripts/deploy-host.sh first (it pushes scripts/node/etc/default/grub.d/*.cfg)"
    fi
